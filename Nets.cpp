...
#endif
template<typename T>
matrix<T> generate_uniform_grid_matrix(double a, double b, int n) {
    if (n < 1) {
...
template<typename T>
matrix<T> generate_chebyshev_grid_matrix(double a, double b, int n) {
    if (n < 1) {
        throw std::invalid_argument("n must be greater than 0.");
    }
    ...
    double factor = M_PI / (2.0 * (n + 1));

    for (int i = 0; i <= n; ++i) {
        // Аргумент косинуса: (2i + 1) * pi / (2(n + 1))
        double theta = (2.0 * i + 1.0) * factor;
